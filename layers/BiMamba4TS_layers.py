...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x):
        # [B, S, D]
        output_forward = self.mamba_forward(x) # (B,S,dim)->(B,S,dim)
        output_forward = self.addnorm_for(output_forward, x) # 残差+归一化

        if self.bi_dir:
            output_backward = self.mamba_backward(x.flip(dims=[1])).flip(dims=[1]) # 逆序输入mamba，然后处理输出
            output_backward = self.addnorm_back(output_backward, x) # 残差+归一化
            output = output_forward + output_backward # 相加/特征融合
        else:
            output = self.addnorm_for(output_forward, x)
        temp = output
        output = self.ffn(output.transpose(-1, 1)).transpose(-1, 1) # (B,S,dim)->(B,dim,S)->ffn->(B,S,dim)
        output = self.addnorm_ffn(output, temp) # 残差+归一化
        return output
