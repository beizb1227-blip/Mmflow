...
# (12, 168, n, 2)
# (12 ,168, 2)
# Fit a GMM on the n samples try 100 and 1000. If no difference pick 100 # 用高斯混合模型对数据分布进行建模，尝试100/1000个进行GMM拟合，如果没有显著差异，则使用100，计算量更小。
# both on rel and absolute
# compare weighted center of the best gmm
# (12,166,1000,2)

def metric(fut_traj, pred_traj):
    # mabs_loss = []
    # kde_loss = []
    # eig_collect = []
    fut_traj = fut_traj.permute(1,0,2) # (12,B,2)
    pred_traj = rearrange(pred_traj, 'b k t p -> t b k p')
    m, nan_list, n_u, m_c, eig = calc_amd_amv(fut_traj.cpu().detach().numpy(),  # 真实未来轨迹（绝对位置）(12,B,2)   预测轨迹（绝对位置） (12,B,20,2)
                                              pred_traj.cpu().detach().numpy())  # m:amd  eig:amv
    # mabs_loss.append(m)  # m   amd
    # eig_collect.append(eig)  # amv
    _kde = kde_lossf(fut_traj.cpu().detach().numpy(), pred_traj.cpu().detach().numpy())  # kde
   #  kde_loss.append(_kde)  #
    return _kde, m, eig
    # return kde_loss, mabs_loss, eig_collect
    # return sum(kde_loss) / len(kde_loss), sum(mabs_loss) / len(mabs_loss), sum(eig_collect) / len(eig_collect)  # kde  amd  amv
